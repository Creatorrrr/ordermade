	                        ...
	                        <tr>
	                            ...
	                            <td style="text-align: center">
	                            	<!-- 수락 ajax 구현 -->
	                           		...
	                           		<!-- 거절 ajax 구현 -->
	                           		<span><input type="button" class="btn btn-default" value="거절" style="display: inline-block;" onclick="javascript:inviteRequestController.rejectInviteRequest('${inviteRequest.id}')"></span>
	                            </td>
	                        </tr>
	                       </c:forEach>
	                   </tbody>
...
var inviteRequestController = {
	acceptInviteRequest : function(requestId, makerId){
		...
		$.ajax({
			...
			success: function(text) {
					if(text === "true"){
						location.href="${ctx}/request/ui/makerInviteList.do?page=1";
					};
			},
			error: function(xml){
				console.log("실패 메시지 :\n" + xml.responseText);
...
function pagination(){
	$.ajax({
		url : "${ctx}/request/pages/makerInviteList.do",
		type : "get",
		dataType : "text",
		success : function(pages) {
		    $('#pagination').pagination({	// 페이지 총 개수를 구한 다음 생성
		        ...
		        onPageClick: function (page, evt) {
		        	location.href = "${ctx }/request/ui/makerInviteList.do?page=" + page;
		        }
		    });
		}
	});
